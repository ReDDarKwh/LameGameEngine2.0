...
var objects = {
  ...
  update : {
    ...
    "projectile": function(inst,dt){
      ...
      inst.ry+= Math.sin(inst.extra.direction)*inst.spd*dt;
    ...
    "enemy": function(inst,dt){
      ...
      if(inst.extra.mode === "smartChase"){
        ...
        var move = objects.typeCollide("enemy",inst,6);
        
        var angle = getAngle(inst.rx+move[0],inst.ry+move[1],world.x*-1,world.y*-1);//(inst.rx,inst.ry,1-world.x,1-world.y);
        ...
        var col = world.col.playerCheckCollisions(inst.x,inst.y,move[0],move[1],6,5), colX = col[0] , colY = col[1];
        ...
        inst.ry += move[1];
         ...
         inst.rx+=Math.cos(inst.extra.angle)*inst.extra.force;
         inst.ry+=Math.sin(inst.extra.angle)*inst.extra.force;
